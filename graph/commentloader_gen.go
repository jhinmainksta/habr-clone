...
type CommentLoader struct {
	...
	cache map[string][]*my_model.Comment
...
func (l *CommentLoader) Load(key string) ([]*my_model.Comment, error) {
...
func (l *CommentLoader) LoadThunk(key string) func() ([]*my_model.Comment, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*my_model.Comment, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CommentLoader) LoadAll(keys []string) ([][]*my_model.Comment, []error) {
	results := make([]func() ([]*my_model.Comment, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	comments := make([][]*my_model.Comment, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *CommentLoader) LoadAllThunk(keys []string) func() ([][]*my_model.Comment, []error) {
	results := make([]func() ([]*my_model.Comment, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *CommentLoader) Prime(key string, value []*my_model.Comment) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*my_model.Comment, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *CommentLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *CommentLoader) unsafeSet(key string, value []*my_model.Comment) {
	if l.cache == nil {
		l.cache = map[string][]*my_model.Comment{}
	}
	l.cache[key] = value
}
...
func (b *commentLoaderBatch) keyIndex(l *CommentLoader, key string) int {
